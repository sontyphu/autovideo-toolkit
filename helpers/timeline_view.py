...
def extract_frames(
    video: Path, start: float, end: float, n: int, dest_dir: Path, width: int = 320
) -> list[Path]:
    """Extract N frames evenly spaced across [start, end]. Returns paths in order.

    `width` defaults to 320 (filmstrip size). Raise it only for --frames-only,
    where the frames are read individually instead of composited.
    """
    ...
    for i, t in enumerate(times):
        ...
        cmd = [
            ...
            "-frames:v", "1",
            "-q:v", "4",
            # Cap height at 1998px so the Read tool can still render the image.
            "-vf", f"scale=w={width}:h='min(1998,ih)':"
...
def frames_only(
    video: Path, start: float, end: float, n: int, dest_dir: Path, width: int
) -> None:
    """Write N standalone frames at `width` px and print their paths.

    The filmstrip composite squeezes every frame to 180px tall, which is fine for
    judging a cut but far too small to READ anything on screen. When the question
    is "what does that filename / command / slide say", skip the composite and
    read the frames individually at full width.
    """
    dest_dir.mkdir(parents=True, exist_ok=True)
    paths = extract_frames(video, start, end, n, dest_dir, width=width)
    step = (end - start) / (n - 1) if n > 1 else 0.0
    print(f"\n{len(paths)} frames at {width}px wide — Read each path to see it:\n")
    for i, p in enumerate(paths):
        t = start + i * step if n > 1 else (start + end) / 2.0
        mm, ss = divmod(int(round(t)), 60)
        print(f"- `{p}` (t={mm:02d}:{ss:02d})")
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default=None,
        metavar="WIDTH",
        help="Skip the composite: write standalone frames at WIDTH px (default 1024) "
             "and print their paths. Use when on-screen TEXT must be readable — the "
             "180px-tall filmstrip cannot show a filename, command, or slide.",
    )
    args = ap.parse_args()
    ...
    if args.frames_only:
        frames_only(
            ...
            n=args.n_frames,
            dest_dir=out_path.parent / f"{video.stem}_{args.start:.2f}-{args.end:.2f}_frames",
            width=args.frames_only,
        )
        return
