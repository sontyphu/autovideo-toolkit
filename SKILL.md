...
- 🇻🇳 **`tim_tu_dem.py <transcript.scribe.json>`** — **dò điểm cắt bằng máy** cho tiếng Việt: từ đệm ("à/ờ/ừm/á"), nói lặp, khoảng im lặng. Ra bảng có **mốc giờ + mức tin cậy** (`chắc` / `xem lại`), `--json` để lấy dữ liệu dựng tiếp, `--chi-chac` chỉ lấy chỗ chắc chắn. **Không tự cắt** - chỉ gợi ý, người quyết.
  **Đã thử và LOẠI BỎ khỏi danh sách từ đệm** (kiểm trên video thật 02/08/2026): `là`, `thì` là **từ nối** không phải đệm (*"tôi sẽ bảo LÀ chạy quảng cáo"*, *"thật tốt THÌ người ta sẽ đến"* - cắt là vỡ câu); `a` không dấu dễ nhầm "A lô"; `hả` là từ hỏi có nghĩa. **Nói lặp chỉ tính trong cùng một câu** - qua dấu chấm là hai câu khác nhau (*"kết nối với tôi. Tôi đến từ..."*).
  Số đo thật: video sự kiện 73 giây → 14 chỗ, cắt hết bỏ 15% thời lượng. Reel 63 giây → 16 chỗ, bỏ 9%.
- 🇻🇳 **`kiem_chat_luong.py <video> --loai <reel|quang-cao|bai-hoc|phim|do-hoa>`** — **chấm chất lượng bằng NGƯỠNG SỐ** trước khi giao. Bảy phép đo: đường tiếng (có/không, âm lượng, đỉnh vỡ) · độ dài theo loại · tỉ lệ và độ phân giải khung · khung đen đầu-cuối · độ sáng · mở đầu có người không (thô, qua vùng màu da). Mỗi lỗi nói rõ **số đo được** và **ngưỡng bị vượt**. Thoát mã 1 khi có lỗi.
  ⚠️ **Chọn đúng `--loai`**: video đồ họa nền tối (gu Xưởng Video AI) chấm nhầm loại `reel` sẽ báo giả "khung đen", "hình tối", "không có người" - vì với đồ họa thì nền tối và không mặt người là **chủ ý**. Dùng `--loai do-hoa`.
  Máy **không bắt được chữ tràn khung** - vẫn phải trích khung hình ra xem (lỗi 9 trong `assets/hyperframes-viet/DOC-TRUOC.md`).
- 🇻🇳 **`lam_thumbnail.py <video> -d <thư-mục> [--chu "CHỮ"] [--gu <tên>]`** — **chọn khung đẹp + làm thumbnail nhiều bản**. Quét N khung đều nhau, chấm điểm theo 4 tiêu chí (có mặt người 40% · nét 30% · sáng vừa 20% · giàu chi tiết 10%), lấy top 10 ứng viên, rồi đắp chữ ra **3 bản bố cục khác nhau**. Màu và font **lấy từ kho gu chung** (`--gu xuong-video-ai`), không phải màu cứng. Video đồ họa thì thêm `--khong-mat-nguoi`.
  ⚠️ **Tiêu chí "có mặt người" là THÔ** - đo vùng màu da, nên **thảm đỏ, tường màu ấm, gỗ cũng lọt lưới**. Điểm 100 không có nghĩa là có mặt người rõ. **Luôn xem 10 khung ứng viên bằng mắt rồi mới chọn.**
- **`timeline_view.py <video> <start> <end>`** — filmstrip + waveform PNG. On-demand visual drill-down. **Not a scan tool** — use it at decision points, not constantly. Add **`--frames-only [WIDTH]`** (default 1024) to skip the composite and write standalone frames at full width, then `Read` each printed path — the only way to read a filename, a command, or slide text, since the composite squeezes frames to 180px tall.
- **`render.py <edl.json> -o <out>`** — per-segment extract → concat → overlays (PTS-shifted) → subtitles LAST. `--preview` for 720p fast. `--build-subtitles` to generate master.srt inline.
- **`grade.py <in> -o <out>`** — ffmpeg filter chain grade. Presets + `--filter '<raw>'` for custom.
- **`tai_video.py <link>`** — **the way in from the internet**, not a viewing tool. Pulls a video plus its transcript off a URL (yt-dlp: YouTube, TikTok, Facebook, Vimeo…) into `downloads/` and prints the local path so the rest of the workshop can take over. `--chi-loi` fetches captions only (~10s, no download, no cost). `--boc-loi` forces ElevenLabs Scribe even when the page has captions. `--khong-boc-loi` keeps a private video off any API. Captions are requested **Vietnamese first**, then English, and the rolling-caption duplication YouTube emits is collapsed (a real run: 303 duplicated blocks → 28 clean ones).

  **HARD RULE — site captions are for READING, Scribe is for CUTTING.** Never feed a `--chi-loi` transcript into a cut plan. Site captions are auto-generated: they mangle proper nouns (one real run turned "Claude" into *clo / claud / claw / cla / lo* and "Haiku" into *Ha cư*), they timestamp **per line (2–3s), not per word**, and they do **not** separate speakers. Cutting off line-level timestamps is exactly how a clip ends up clipped mid-word. Use them to skim, locate a section, or study someone else's structure. The moment the output is a cut, a subtitle burn, or a quote, re-transcribe with Scribe (`--boc-loi`, or `transcribe.py` directly). This is the same failure mode as Hard Rule 13, one tier worse.

  **Three tools, three jobs — they do not overlap:**
  | Need | Tool |
  |---|---|
  | Get a video off the internet onto this machine | `tai_video.py` — then use the two below on the downloaded file |
  | Judge a cut point | `timeline_view.py` — filmstrip **plus waveform and word labels**; `--frames-only` when on-screen text must be legible |
  | Learn a style from a reference video (pacing, caption position, cuts per minute) | `soi_phong_cach.py` in skill `hieu-ung-video-thuong-hieu` — **measures** it, one frame per scene, numeric report |
